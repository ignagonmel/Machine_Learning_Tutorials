...
class TaxiEnv(Env):
    ...
    * v0: Initial version release
    ...
    def __init__(self, desc: Optional[str] = None, render_mode: Optional[str] = None):
        if desc is None:
            self.desc = [
                        "+---------+",
                        "|R: | : :G|",
        ...
        self.locs, self.locs_colors = self.get_locations_and_colors() # [(0, 0), (0, 4), (4, 0), (4, 3)]
    ...
    def get_locations_and_colors(self):
        """Obtains locations from description and colors"""
        possible_colors = self.tiles_colors.keys()
        locs = []
        colors = []
        no_borders = self.desc[1:-1, 1:-1]
        no_obstacles = no_borders[:, ::2]
        
        for color in possible_colors:
            row, col = np.where(no_obstacles == color.encode("utf-8"))
            position = list(zip(row, col))
            if len(position) > 0:
                locs.append(position[0])
                colors.append(self.tiles_colors[color])
